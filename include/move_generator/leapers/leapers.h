...
private:
    static inline void addMoves(MoveList& move_list, int source_square, u64 moves, MoveFlag flag);
...
inline void initLeapers()
{
    ...
    for ( int i = 0; i < 64; ++i ) {
        white_pawn_attacks[i] = leapers::generatePawnMask<type::Color::white>(single_bit_u64(i));;
        black_pawn_attacks[i] = leapers::generatePawnMask<type::Color::black>(single_bit_u64(i));
